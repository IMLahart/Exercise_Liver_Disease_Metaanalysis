...
```{r Forest plot AST, fig.width = 10, fig.height=7}
meta::forest(m.AST, 
             layout = "RevMan5",
             digits.mean = 1,
             digits.sd = 1,
...
pdf(file = "AST_forestplot.pdf", width = 10, height = 7)
...
m.AST.inf <- metainf(m.AST, pooled = "random") # Leave one out analysis

m.AST.inf
```

```{r Forest plot leave one out analysis AST, fig.width=10, fig.height=6}
forest(metainf(m.AST), layout = "revman5")
```
...
```{r Create effect sizes for AST rma dataset}
df_AST <- metafor::escalc(measure = "MD",
                 m1i = mean.e,
                 m2i = mean.c,
                 sd1i = sd.e,
                 sd2i = sd.c,
...
### Subgroup analyses AST

```{r Subgroup analysis by disease type AST}
Disease_subg <- update(m.AST, 
                    subgroup = Disease_type, 
                    tau.common = FALSE)
```

```{r Forest plot disease type subgoup AST, fig.width=10, fig.height=9}
forest(Disease_subg,
            layout = "Revman",
             digits.mean = 1,
             digits.sd = 1,
...
```{r Forest plot region type subgoup AST, fig.height=8, fig.width=10}
forest(Region_subg,
            layout = "Revman",
            digits.mean = 1,
            digits.sd = 1,
...
```{r Bubble plot AST, fig.height=5, fig.width=8}
...
## Meta-analysis: Glucose

```{r Read in glucose data, include=FALSE}
Liver_disease_glucose <- read.csv("MA_human_glucose.csv", 
                      header = TRUE, sep = ",")
```

```{r glucose meta-analysis}
m.glucose <- metacont(n.e = n.e,
                   mean.e = mean.e,
                   sd.e = sd.e,
                   n.c = n.c,
                   mean.c = mean.c,
                   ...
                   method.tau = "DL",
                   method.random.ci = "HK",
                   title = "Exercise effects on glucose")

m.glucose <- update(m.glucose, prediction = TRUE) # Adds the prediction interval

summary(m.glucose) # Results of meta-analysis
```

```{r Outlier search glucose}
find.outliers(m.glucose) # searches for outlying studies in a object, removes them, and then recalculates the results.
```

Influential cases, on the other hand, are those studies which–by definition–have a large impact on the pooled effect or heterogeneity, regardless of how high or low the effect is. 
Influence diagnostics allow us to detect the studies which influence the overall estimate of our meta-analysis the most.They let us assess if this large influence distorts our pooled effect.

```{r Influential cases glucose}
m.glucose.inf <- InfluenceAnalysis(m.glucose, random = TRUE) #Influence diagnostics allow us to detect the studies that distorts the pooled effect
```
...
```{r Influence analysis plots glucose}
plot(m.glucose.inf, "baujat")
plot(m.glucose.inf, "influence")
plot(m.glucose.inf, "es")
plot(m.glucose.inf, "i2")
```

Graphic Display of Heterogeneity (GOSH) plots fit the same meta-analysis model to all possible subsets of our included studies.

```{r Gosh plot glucose}
m.rma_glucose <- rma(yi = m.glucose$TE,
             sei = m.glucose$seTE,
             method = m.glucose$method.tau,
             test = "knha")

res.gosh_glucose <- gosh(m.rma_glucose)

plot(res.gosh_glucose, alpha = 0.01)

res.gosh.diag_glucose <- gosh.diagnostics(res.gosh_glucose, 
                                  km.params = list(centers = 2),
                                  db.params = list(eps = 0.08, 
                                                   MinPts = 50))
res.gosh.diag

plot(res.gosh.diag_glucose)
```

```{r Update analysis if necessary}
# update(m.glucose, exclude = c(3, 4)) %>% 
  # summary()
```

```{r Forest plot glucose}
meta::forest(m.glucose, layout = "RevMan5")
```

```{r Saving forest plot glucose}
pdf(file = "glucose_forestplot.pdf", width = 10, height = 7)
...
```{r Metaregression glucose}
m.glucose.reg <- metareg(m.glucose, ~Age)
m.glucose.reg
```


```{r Bubble plot glucose}
bubble(m.glucose.reg, studlab = TRUE)
```

### Publication bias

```{r Funnel plot glucose}
meta::funnel(m.glucose,
             xlim = c(-0.5, 2),
             studlab = TRUE)

# Add title
title("Funnel Plot (glucose)")
```

```{r Contour funnel plot glucose}
# Define fill colors for contour
col.contour = c("gray75", "gray85", "gray95")

# Generate funnel plot (we do not include study labels here)
meta::funnel(m.glucose, xlim = c(-0.5, 2),
             contour = c(0.9, 0.95, 0.99),
             col.contour = col.contour)
...
# Add a title
title("Contour-Enhanced Funnel Plot (glucose)")
```

```{r Egger's regression test}
metabias(m.glucose, method.bias = "linreg")

# Alternative:
m.glucose$n.e = n1; m.glucose$n.c = n2
metabias(m.glucose, method.bias = "Pustejovsky")
```

```{r p-curve analysis}
# Update m.gen to exclude outliers
# m.glucose_update <- update(m.glucose, subset = -c(3, 16))

# Run p-curve analysis
pcurve(m.glucose_update)
```
...
## Meta-analysis: Insulin 

```{r Read in insulin data, include=FALSE}
Liver_disease_insulin <- read.csv("MA_human_insulin.csv", 
                      header = TRUE, sep = ",")
```

```{r insulin meta-analysis}
m.insulin <- metacont(n.e = n.e,
                   mean.e = mean.e,
                   sd.e = sd.e,
                   n.c = n.c,
                   mean.c = mean.c,
                   ...
                   method.tau = "DL",
                   method.random.ci = "HK",
                   title = "Exercise effects on insulin")

m.insulin <- update(m.insulin, prediction = TRUE) # Adds the prediction interval

summary(m.insulin) # Results of meta-analysis
```

```{r Outlier search insulin}
find.outliers(m.insulin) # searches for outlying studies in a object, removes them, and then recalculates the results.
```

Influential cases, on the other hand, are those studies which–by definition–have a large impact on the pooled effect or heterogeneity, regardless of how high or low the effect is. 
Influence diagnostics allow us to detect the studies which influence the overall estimate of our meta-analysis the most.They let us assess if this large influence distorts our pooled effect.

```{r Influential cases insulin}
m.insulin.inf <- InfluenceAnalysis(m.insulin, random = TRUE) #Influence diagnostics allow us to detect the studies that distorts the pooled effect
```
...
```{r Influence analysis plots insulin}
plot(m.insulin.inf, "baujat")
plot(m.insulin.inf, "influence")
plot(m.insulin.inf, "es")
plot(m.insulin.inf, "i2")
```

Graphic Display of Heterogeneity (GOSH) plots fit the same meta-analysis model to all possible subsets of our included studies.

```{r Gosh plot insulin}
m.rma_insulin <- rma(yi = m.insulin$TE,
             sei = m.insulin$seTE,
             method = m.insulin$method.tau,
             test = "knha")

res.gosh_insulin <- gosh(m.rma_insulin)

plot(res.gosh_insulin, alpha = 0.01)

res.gosh.diag_insulin <- gosh.diagnostics(res.gosh_insulin, 
                                  km.params = list(centers = 2),
                                  db.params = list(eps = 0.08, 
                                                   MinPts = 50))
res.gosh.diag

plot(res.gosh.diag_insulin)
```

```{r Update analysis if necessary}
# update(m.insulin, exclude = c(3, 4)) %>% 
  # summary()
```

```{r Forest plot insulin}
meta::forest(m.insulin, layout = "RevMan5")
```

```{r Saving forest plot insulin}
pdf(file = "insulin_forestplot.pdf", width = 10, height = 7)
...
```{r Metaregression insulin}
m.insulin.reg <- metareg(m.insulin, ~Age)
m.insulin.reg
```


```{r Bubble plot insulin}
bubble(m.insulin.reg, studlab = TRUE)
```

### Publication bias

```{r Funnel plot insulin}
meta::funnel(m.insulin,
             xlim = c(-0.5, 2),
             studlab = TRUE)

# Add title
title("Funnel Plot (insulin)")
```

```{r Contour funnel plot insulin}
# Define fill colors for contour
col.contour = c("gray75", "gray85", "gray95")

# Generate funnel plot (we do not include study labels here)
meta::funnel(m.insulin, xlim = c(-0.5, 2),
             contour = c(0.9, 0.95, 0.99),
             col.contour = col.contour)
...
# Add a title
title("Contour-Enhanced Funnel Plot (insulin)")
```

```{r Egger's regression test}
metabias(m.insulin, method.bias = "linreg")

# Alternative:
m.insulin$n.e = n1; m.insulin$n.c = n2
metabias(m.insulin, method.bias = "Pustejovsky")
```

```{r p-curve analysis}
# Update m.gen to exclude outliers
# m.insulin_update <- update(m.insulin, subset = -c(3, 16))

# Run p-curve analysis
pcurve(m.insulin_update)
```
...
## Meta-analysis: Visceral_adiposity 

```{r Read in visceral_adipose data, include=FALSE}
Liver_disease_visceral_adipose <- read.csv("MA_human_visceral_adipose.csv", 
                      header = TRUE, sep = ",")
```

```{r visceral_adipose meta-analysis}
m.visceral_adipose <- metacont(n.e = n.e,
                   mean.e = mean.e,
                   sd.e = sd.e,
                   n.c = n.c,
                   mean.c = mean.c,
                   sd.c = sd.c,
                   studlab = author,
                   data = Liver_disease_visceral_adipose,
                   sm = "SMD",
                   method.smd = "Hedges",
                   fixed = FALSE,
                   random = TRUE,
                   method.tau = "DL",
                   method.random.ci = "HK",
                   title = "Exercise effects on visceral_adipose")

m.visceral_adipose <- update(m.visceral_adipose, prediction = TRUE) # Adds the prediction interval

summary(m.visceral_adipose) # Results of meta-analysis
```

```{r Outlier search visceral_adipose}
find.outliers(m.visceral_adipose) # searches for outlying studies in a object, removes them, and then recalculates the results.
```

Influential cases, on the other hand, are those studies which–by definition–have a large impact on the pooled effect or heterogeneity, regardless of how high or low the effect is. 
Influence diagnostics allow us to detect the studies which influence the overall estimate of our meta-analysis the most.They let us assess if this large influence distorts our pooled effect.

```{r Influential cases visceral_adipose}
m.visceral_adipose.inf <- InfluenceAnalysis(m.visceral_adipose, random = TRUE) #Influence diagnostics allow us to detect the studies that distorts the pooled effect
```
...
```{r Influence analysis plots visceral_adipose}
plot(m.visceral_adipose.inf, "baujat")
plot(m.visceral_adipose.inf, "influence")
plot(m.visceral_adipose.inf, "es")
plot(m.visceral_adipose.inf, "i2")
```

Graphic Display of Heterogeneity (GOSH) plots fit the same meta-analysis model to all possible subsets of our included studies.

```{r Gosh plot visceral_adipose}
m.rma_visceral_adipose <- rma(yi = m.visceral_adipose$TE,
             sei = m.visceral_adipose$seTE,
             method = m.visceral_adipose$method.tau,
             test = "knha")

res.gosh_visceral_adipose <- gosh(m.rma_visceral_adipose)

plot(res.gosh_visceral_adipose, alpha = 0.01)

res.gosh.diag_visceral_adipose <- gosh.diagnostics(res.gosh_visceral_adipose, 
                                  km.params = list(centers = 2),
                                  db.params = list(eps = 0.08, 
                                                   MinPts = 50))
res.gosh.diag

plot(res.gosh.diag_visceral_adipose)
```

```{r Update analysis if necessary}
# update(m.visceral_adipose, exclude = c(3, 4)) %>% 
  # summary()
```

```{r Forest plot visceral_adipose}
meta::forest(m.visceral_adipose, layout = "RevMan5")
```

```{r Saving forest plot visceral_adipose}
pdf(file = "visceral_adipose_forestplot.pdf", width = 11, height = 7)
...
```{r Subgroup analysis by disease type}
Disease_subg <- update(m.visceral_adipose, 
                    subgroup = Disease_type, 
                    tau.common = FALSE)
```
...
```{r Metaregression visceral_adipose}
m.visceral_adipose.reg <- metareg(m.visceral_adipose, ~Age)
m.visceral_adipose.reg
```


```{r Bubble plot visceral_adipose}
bubble(m.visceral_adipose.reg, studlab = TRUE)
```

### Publication bias

```{r Funnel plot visceral_adipose}
meta::funnel(m.visceral_adipose,
             xlim = c(-0.5, 2),
             studlab = TRUE)

# Add title
title("Funnel Plot (visceral_adipose)")
```

```{r Contour funnel plot visceral_adipose}
# Define fill colors for contour
col.contour = c("gray75", "gray85", "gray95")

# Generate funnel plot (we do not include study labels here)
meta::funnel(m.visceral_adipose, xlim = c(-0.5, 2),
             contour = c(0.9, 0.95, 0.99),
             col.contour = col.contour)
...
# Add a title
title("Contour-Enhanced Funnel Plot (visceral_adipose)")
```

```{r Egger's regression test}
metabias(m.visceral_adipose, method.bias = "linreg")

# Alternative:
m.visceral_adipose$n.e = n1; m.visceral_adipose$n.c = n2
metabias(m.visceral_adipose, method.bias = "Pustejovsky")
```

```{r p-curve analysis}
# Update m.gen to exclude outliers
# m.visceral_adipose_update <- update(m.visceral_adipose, subset = -c(3, 16))

# Run p-curve analysis
pcurve(m.visceral_adipose_update)
```
...
## Meta-analysis: Subcutaneous_adiposity 

```{r Read in subcutaneous_adipose data, include=FALSE}
Liver_disease_subcutaneous_adipose <- read.csv("MA_human_subcutaneous_adipose.csv", 
                      header = TRUE, sep = ",")
```

```{r subcutaneous_adipose meta-analysis}
m.subcutaneous_adipose <- metacont(n.e = n.e,
                   mean.e = mean.e,
                   sd.e = sd.e,
                   n.c = n.c,
                   mean.c = mean.c,
                   sd.c = sd.c,
                   studlab = author,
                   data = Liver_disease_subcutaneous_adipose,
                   sm = "SMD",
                   method.smd = "Hedges",
                   fixed = FALSE,
                   random = TRUE,
                   method.tau = "DL",
                   method.random.ci = "HK",
                   title = "Exercise effects on subcutaneous_adipose")

m.subcutaneous_adipose <- update(m.subcutaneous_adipose, prediction = TRUE) # Adds the prediction interval

summary(m.subcutaneous_adipose) # Results of meta-analysis
```

```{r Outlier search subcutaneous_adipose}
find.outliers(m.subcutaneous_adipose) # searches for outlying studies in a object, removes them, and then recalculates the results.
```

Influential cases, on the other hand, are those studies which–by definition–have a large impact on the pooled effect or heterogeneity, regardless of how high or low the effect is. 
Influence diagnostics allow us to detect the studies which influence the overall estimate of our meta-analysis the most.They let us assess if this large influence distorts our pooled effect.

```{r Influential cases subcutaneous_adipose}
m.subcutaneous_adipose.inf <- InfluenceAnalysis(m.subcutaneous_adipose, random = TRUE) #Influence diagnostics allow us to detect the studies that distorts the pooled effect
```
...
```{r Influence analysis plots subcutaneous_adipose}
plot(m.subcutaneous_adipose.inf, "baujat")
plot(m.subcutaneous_adipose.inf, "influence")
plot(m.subcutaneous_adipose.inf, "es")
plot(m.subcutaneous_adipose.inf, "i2")
```

Graphic Display of Heterogeneity (GOSH) plots fit the same meta-analysis model to all possible subsets of our included studies.

```{r Gosh plot subcutaneous_adipose}
m.rma_subcutaneous_adipose <- rma(yi = m.subcutaneous_adipose$TE,
             sei = m.subcutaneous_adipose$seTE,
             method = m.subcutaneous_adipose$method.tau,
             test = "knha")

res.gosh_subcutaneous_adipose <- gosh(m.rma_subcutaneous_adipose)

plot(res.gosh_subcutaneous_adipose, alpha = 0.01)

res.gosh.diag_subcutaneous_adipose <- gosh.diagnostics(res.gosh_subcutaneous_adipose, 
                                  km.params = list(centers = 2),
                                  db.params = list(eps = 0.08, 
                                                   MinPts = 50))
res.gosh.diag

plot(res.gosh.diag_subcutaneous_adipose)
```

```{r Update analysis if necessary}
# update(m.subcutaneous_adipose, exclude = c(3, 4)) %>% 
  # summary()
```

```{r Forest plot subcutaneous_adipose}
meta::forest(m.subcutaneous_adipose, layout = "RevMan5")
```

```{r Saving forest plot subcutaneous_adipose}
pdf(file = "subcutaneous_adipose_forestplot.pdf", width = 11, height = 7)
...
```{r Subgroup analysis by disease type}
Disease_subg <- update(m.subcutaneous_adipose, 
                    subgroup = Disease_type, 
                    tau.common = FALSE)
```
...
```{r Metaregression subcutaneous_adipose}
m.subcutaneous_adipose.reg <- metareg(m.subcutaneous_adipose, ~Age)
m.subcutaneous_adipose.reg
```


```{r Bubble plot subcutaneous_adipose}
bubble(m.subcutaneous_adipose.reg, studlab = TRUE)
```

### Publication bias

```{r Funnel plot subcutaneous_adipose}
meta::funnel(m.subcutaneous_adipose,
             xlim = c(-0.5, 2),
             studlab = TRUE)

# Add title
title("Funnel Plot (subcutaneous_adipose)")
```

```{r Contour funnel plot subcutaneous_adipose}
# Define fill colors for contour
col.contour = c("gray75", "gray85", "gray95")

# Generate funnel plot (we do not include study labels here)
meta::funnel(m.subcutaneous_adipose, xlim = c(-0.5, 2),
             contour = c(0.9, 0.95, 0.99),
             col.contour = col.contour)
...
# Add a title
title("Contour-Enhanced Funnel Plot (subcutaneous_adipose)")
```

```{r Egger's regression test}
metabias(m.subcutaneous_adipose, method.bias = "linreg")

# Alternative:
m.subcutaneous_adipose$n.e = n1; m.subcutaneous_adipose$n.c = n2
metabias(m.subcutaneous_adipose, method.bias = "Pustejovsky")
```

```{r p-curve analysis}
# Update m.gen to exclude outliers
# m.subcutaneous_adipose_update <- update(m.subcutaneous_adipose, subset = -c(3, 16))

# Run p-curve analysis
pcurve(m.subcutaneous_adipose_update)
```
...
```{r Read in HbA1c data, include=FALSE}
Liver_disease_HbA1c <- read.csv("MA_human_HbA1c.csv", 
                      header = TRUE, sep = ",")
```

```{r HbA1c meta-analysis}
m.HbA1c <- metacont(n.e = n.e,
                   mean.e = mean.e,
                   sd.e = sd.e,
                   n.c = n.c,
                   mean.c = mean.c,
                   ...
                   method.tau = "DL",
                   method.random.ci = "HK",
                   title = "Exercise effects on HbA1c")

m.HbA1c <- update(m.HbA1c, prediction = TRUE) # Adds the prediction interval

summary(m.HbA1c) # Results of meta-analysis
```

```{r Outlier search HbA1c}
find.outliers(m.HbA1c) # searches for outlying studies in a object, removes them, and then recalculates the results.
```

Influential cases, on the other hand, are those studies which–by definition–have a large impact on the pooled effect or heterogeneity, regardless of how high or low the effect is. 
Influence diagnostics allow us to detect the studies which influence the overall estimate of our meta-analysis the most.They let us assess if this large influence distorts our pooled effect.

```{r Influential cases HbA1c}
m.HbA1c.inf <- InfluenceAnalysis(m.HbA1c, random = TRUE) #Influence diagnostics allow us to detect the studies that distorts the pooled effect
```
...
```{r Influence analysis plots HbA1c}
plot(m.HbA1c.inf, "baujat")
plot(m.HbA1c.inf, "influence")
plot(m.HbA1c.inf, "es")
plot(m.HbA1c.inf, "i2")
```
...
```{r Gosh plot HbA1c}
m.rma_HbA1c <- rma(yi = m.HbA1c$TE,
             sei = m.HbA1c$seTE,
             method = m.HbA1c$method.tau,
             test = "knha")

res.gosh_HbA1c <- gosh(m.rma_HbA1c)

plot(res.gosh_HbA1c, alpha = 0.01)

res.gosh.diag_HbA1c <- gosh.diagnostics(res.gosh_HbA1c, 
                                  km.params = list(centers = 2),
                                  db.params = list(eps = 0.08, 
                                                   MinPts = 50))
res.gosh.diag

plot(res.gosh.diag_HbA1c)
```

```{r Update analysis if necessary}
# update(m.HbA1c, exclude = c(3, 4)) %>% 
  # summary()
```

```{r Forest plot HbA1c}
meta::forest(m.HbA1c, layout = "RevMan5")
```

```{r Saving forest plot HbA1c}
pdf(file = "HbA1c_forestplot.pdf", width = 10, height = 7)
...
```{r Metaregression HbA1c}
m.HbA1c.reg <- metareg(m.HbA1c, ~Age)
m.HbA1c.reg
```


```{r Bubble plot HbA1c}
bubble(m.HbA1c.reg, studlab = TRUE)
```
...
# Add title
title("Funnel Plot (HbA1c)")
```

```{r Contour funnel plot HbA1c}
# Define fill colors for contour
col.contour = c("gray75", "gray85", "gray95")

# Generate funnel plot (we do not include study labels here)
meta::funnel(m.HbA1c, xlim = c(-0.5, 2),
             contour = c(0.9, 0.95, 0.99),
             col.contour = col.contour)
...
# Add a title
title("Contour-Enhanced Funnel Plot (HbA1c)")
```

```{r Egger's regression test}
metabias(m.HbA1c, method.bias = "linreg")

# Alternative:
m.HbA1c$n.e = n1; m.HbA1c$n.c = n2
metabias(m.HbA1c, method.bias = "Pustejovsky")
```

```{r p-curve analysis}
# Update m.gen to exclude outliers
# m.HbA1c_update <- update(m.HbA1c, subset = -c(3, 16))

# Run p-curve analysis
pcurve(m.HbA1c_update)
```
